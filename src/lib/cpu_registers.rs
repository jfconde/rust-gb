...
impl CPURegisters {
    ...
    pub fn get_flg (&mut self) -> &mut CPUFlags { &mut self.flg }
    ...
    pub fn inc_pc (&mut self) -> u16 { self.pc = self.pc.wrapping_add(1); self.pc }
    pub fn dec_pc (&mut self) -> u16 { self.pc = self.pc.wrapping_sub(1); self.pc }
